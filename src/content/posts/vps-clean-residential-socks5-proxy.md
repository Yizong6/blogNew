...
:::warning
VPNGate 是公共志愿节点池，无法保证 IP 一定“纯净”、一定是家宽、一定独享或长期稳定。本文中的“类家宽出口”指的是通过筛选 ISP、ASN、PTR 等信息，尽量选择看起来像家庭宽带的出口节点。
:::
...
```bash
curl --proxy socks5h://127.0.0.1:7928 https://api.ipify.org ; echo
```
...
```bash
curl -x http://127.0.0.1:7928 https://api.ipify.org ; echo
```
...
```bash
sudo tee /opt/auth-socks-aimili.py >/dev/null <<'EOF'
#!/usr/bin/env python3
import select
import socket
import threading
...
```bash
curl --proxy socks5h://yizong:yizong@127.0.0.1:52000 https://api.ipify.org ; echo
```
...
```bash
curl --proxy socks5h://wrong:wrong@127.0.0.1:52000 https://api.ipify.org ; echo
```
